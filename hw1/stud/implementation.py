...
class StudentModel(Model):
    ...
    def __init__(self):
        ...
        self.pretrained_emb = load_pretrained_embedding(self.dataset.word_to_idx)

        # load pre-trained model from file
        self.model = load_saved_model(
            save_path="model/bests/rnn70_glove50d_4.pt",
    ...
    def predict(self, sentence_pairs: List[Dict]) -> List[str]:
        ...
        for step in range(0,len(sentence_pairs), self._step_size):
            ...
            for pair in step_pairs:
                elem = indexify(
                    spair=pair, 
                    word_to_idx=self.dataset.word_to_idx,
                    unk_token=self.unk_token,
                    sep_token=self.sep_token,
